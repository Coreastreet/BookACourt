...
goog.TRUSTED_TYPES_POLICY_NAME?goog.createTrustedTypesPolicy(goog.TRUSTED_TYPES_POLICY_NAME+"#base"):null,goog.global.CLOSURE_NO_DEPS||goog.debugLoader_.loadClosureDeps(),goog.bootstrap=function(a,b){goog.debugLoader_.bootstrap(a,b)});goog.TRUSTED_TYPES_POLICY_NAME="";goog.identity_=function(a){return a};goog.createTrustedTypesPolicy=function(a){var b=null,c=goog.global.trustedTypes;if(!c||!c.createPolicy)return b;try{b=c.createPolicy(a,{createHTML:goog.identity_,createScript:goog.identity_,createScriptURL:goog.identity_})}catch(d){goog.logToConsole_(d.message)}return b};document.addEventListener("DOMContentLoaded",function(){if(document.querySelector("#manage_peak_hours")){var a=function(){var a="",b="",e=0,f=[];$(".times").each(function(a){0<$(this).text().trim().length&&(a=$(this).text().substring(5).split("-"),b='"'+$(this).attr("id")+'":{ "startingPeakHour":"'+a[0]+'", "closingPeakHour":"'+a[1]+'"}',f.push(b),e++)});return a="{"+f.join(", ")+"}"},b=$(".sports_centre_holder").data("sports-centre-id");$("body").on("click",".manage_peak_hours",function(){$(".peak-times").removeClass("d-none");
$(".court-times").addClass("d-none");$(".courts-table").removeClass("courts-table").addClass("days-table");var a=$(".table tr")[0].children.length-1,b;$(".table tr").each(function(c){for(b=a;1<=b;)$(this).children()[b].remove(),b--});var e=[];$("th sup").each(function(){e.push($(this).text())});var f="Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),g="Sun Mon Tue Wed Thu Fri Sat".split(" "),h;console.log(e);$(".table tr").each(function(a){h=0;if(0==a)for(;7>h;)$(this).append('<th scope="col" class="equalTH">'+
f[h]+"</th>"),h++;else for(;7>h;)$(this).append('<td class="peakSelector" data-time="'+e[a-1]+'" data-day="'+g[h]+'">&nbsp;</td>'),h++});$("th.equalTH").each(function(a){$(this).width($("td.peakSelector").first().width())});$("#opening_hours").prop("id","peak_hours").text("Peak hours;");$(".peak_hour_buttons").removeClass("d-none").addClass("d-flex")});$("body").on("click",".confirm",function(){var c=a();$.ajax({type:"POST",url:b+"/peak_hours",data:{peak_hours:c},success:function(a){alert("New Peak Hours Saved!")},
error:function(a){alert()}})});window.addEventListener("resize",function(){$(".moreEqualTH").css("min-width",$("th[scope='row']")[0].offsetWidth);$("th.equalTH").each(function(a){$(this).css("max-width",$("td.peakSelector").first().width());$(this).css("word-wrap","break-word")})})}});
