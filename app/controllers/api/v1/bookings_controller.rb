class Api::V1::BookingsController < Api::V1::ApiController
  ...
  def check_availability
    ...
    if !temp_reservations.nil?
        ...
        temp_reservations_array.select!{ |booking| (Time.now - booking.created_at) < 2.minutes } # remove those reservations stored longer than 2mins to prevent clogging up redis.
  ...
  def reserve
    ...
    RestClient.post "https://weball.com.au/pub/#{id}",  {event: "live_reservation_update", bookings: jsonBookingArray}.to_json, {content_type: :json, accept: :json}
    if bookingArray.any?
